...
def shear_x(img, v):
    ...
    warp_fn = functional._maybe_process_in_chunks(cv2.warpAffine, M=matrix, dsize=(width, height))
    return warp_fn(img)
...
TRANSFORMS_DICT = {
    ...
    "shearx": (ShearX, 0.0, 0.3),  # 0
    "sheary": (ShearY, 0.0, 0.3),  # 1
    "translatex": (TranslateX, 0.0, 0.3),  # 2
    "translatey": (TranslateY, 0.0, 0.3),  # 3
    "rotate": (Rotate, 0, 30),  # 4
    # (Cutout, 0, 0.5),  # 14
    "flip": (Flip, 0, 1),  # 15
    "autocontrast": (AutoContrast, 0, 1),  # 5
    "invert": (Invert, 0, 1),  # 6
    "equalize": (Equalize, 0, 1),  # 7
    "solarize": (Solarize, 0, 255),  # 8
    "posterize": (Posterize, 4, 8),  # 9
    "contrast": (Contrast, 0.05, 0.95),  # 10
    "color": (Color, 0.05, 0.95),  # 11
    "brightness": (Brightness, 0.05, 0.95),  # 12
    "sharpness": (Sharpness, 0.05, 0.95),  # 13
}
...
class RandAugment(object):
    ...
    def __init__(self, n, m, transforms_list=None):
        self.n = n
        self.m = m
        self.augment_list = [
            TRANSFORMS_DICT[name] for name in transforms_list
        ] if transforms_list else list(TRANSFORMS_DICT.values())
...
class RandAugmentCutout(RandAugment):
    def __call__(self, img):
        ops = random.sample(self.augment_list, k=self.n)
